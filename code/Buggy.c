...
void app_main(void)
{
    ...
    while (1)
    {
        ...
        displaybuffer[2] = alphafonttable[16];
        displaybuffer[3] = alphafonttable[16];
        displaybuffer[1] = 0b0100000000000000;
        ...
        if (current_distance > 80)
        {
            ...
            ESP_LOGI(TAG, "Obstacle Ahead %fcm away", current_distance);
        }
        else
        {
            ESP_ERROR_CHECK(mcpwm_comparator_set_compare_value(comparator2, ESC_SPEED(-0.5)));
            ESP_LOGI(TAG, "Obstacle Detected %fcm away", current_distance);
        }
